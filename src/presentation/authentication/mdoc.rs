...
pub fn w3c_device_authentication(
    ...
    println!("[w3c_device_auth] start, doc_type={:?}", document.doc_type);
    ...
    println!("[w3c_device_auth] base_jwt length={}", base_jwt.len());

    let TokenSlices{claims,..} = raw::split_token(base_jwt).map_err(|e| {
        println!("[w3c_device_auth] failed to split JWT: {e:?}");
        Error::ParsingError
    })?;
    let raw_claim = raw::decode_json_token_slice(claims).map_err(|e| {
        println!("[w3c_device_auth] failed to decode JWT payload: {e:?}");
        Error::ParsingError
    })?;
    let payload_object = raw_claim.as_object().ok_or_else(|| {
        println!("[w3c_device_auth] JWT payload is not a JSON object");
        Error::ParsingError
    })?;
    println!("[w3c_device_auth] JWT payload keys: {:?}", payload_object.keys().collect::<Vec<_>>());
    ...
    let binding_key_jwk: SsiJwk = if let Some(cnf) = payload_object.get("cnf") {
        println!("[w3c_device_auth] key binding path: cnf.jwk");
        let jwk_val = cnf.get("jwk").ok_or_else(|| {
            println!("[w3c_device_auth] cnf present but no jwk field; cnf={cnf}");
            Error::ParsingError
        })?;
        serde_json::from_value(jwk_val.clone()).map_err(|e| {
            println!("[w3c_device_auth] failed to parse cnf.jwk as SsiJwk: {e:?}");
            Error::ParsingError
        })?
    } else {
        println!("[w3c_device_auth] key binding path: vc.credentialSubject.id (did:jwk)");
        let vc = payload_object.get("vc").and_then(|v| v.as_object()).ok_or_else(|| {
            println!("[w3c_device_auth] no cnf and no vc claim in payload");
            Error::ParsingError
        })?;
        let credential_subject = vc.get("credentialSubject").and_then(|v| v.as_object()).ok_or_else(|| {
            println!("[w3c_device_auth] no credentialSubject in vc");
            Error::ParsingError
        })?;
        let jwk_did = credential_subject.get("id").and_then(|v| v.as_str()).ok_or_else(|| {
            println!("[w3c_device_auth] no id in credentialSubject");
            Error::ParsingError
        })?;
        println!("[w3c_device_auth] credentialSubject.id={jwk_did:?}");
        let key_part = jwk_did.get(8..).ok_or_else(|| {
            println!("[w3c_device_auth] id does not start with did:jwk:");
            Error::ParsingError
        })?;
        let jwk_bytes = base64_url::decode(key_part).map_err(|e| {
            println!("[w3c_device_auth] base64url decode of did:jwk key failed: {e:?}");
            Error::ParsingError
        })?;
        let jwk_str = String::from_utf8(jwk_bytes).map_err(|e| {
            println!("[w3c_device_auth] did:jwk bytes are not valid utf-8: {e:?}");
            Error::ParsingError
        })?;
        SsiJwk::from_str(&jwk_str).map_err(|e| {
            println!("[w3c_device_auth] failed to parse did:jwk JWK: {e:?}");
            Error::ParsingError
        })?
    };

    println!("[w3c_device_auth] binding key extracted, params type={}", match &binding_key_jwk.params {
        Params::EC(_) => "EC",
        _ => "other",
    });
    ...
    match binding_key_jwk.params {
        Params::EC(p) => {
            ...
            let (Some(x), Some(y)) = (x_coordinate, y_coordinate) else {
                println!("[w3c_device_auth] EC key missing x or y coordinate");
            ...
            let verifying_key = VerifyingKey::from_encoded_point(&encoded_point).map_err(|e| {
                println!("[w3c_device_auth] failed to build P-256 verifying key: {e:?}");
                Error::from(e)
            })?;
            println!("[w3c_device_auth] P-256 verifying key built successfully");
            ...
            match device_auth {
                DeviceAuth::DeviceSignature(device_signature) => {
                    ...
                    if !result.is_success() {
                        println!("[w3c_device_auth] COSE signature verification failed");
                        Err(Error::ParsingError)?
                    } else {
                        println!("[w3c_device_auth] COSE signature verification succeeded");
                        Ok(())
                    }
                }
                DeviceAuth::DeviceMac(_) => {
                    println!("[w3c_device_auth] DeviceMac not supported");
                    Err(Error::Unsupported)
                }
            }
        }
        _ => {
            println!("[w3c_device_auth] binding key is not EC type");
...
pub fn ldp_vc_device_authentication(
    ...
    // The issuer binds the holder key into credentialSubject.id as a did:jwk: URI;
    // the holder proves control by signing the VP with that same key.
    let inner_vc = vp
